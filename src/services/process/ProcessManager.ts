...
export class ProcessManager {
  ...
  private static async startWithBun(script: string, logFile: string, port: number): Promise<{ success: boolean; pid?: number; error?: string }> {
    ...
    try {
      ...
      if (isWindows) {
        // Windows: Use PowerShell with Base64-encoded command (prevents command injection)
        // This properly hides the console window and adds logging support
        ...
        // Escape backslashes in log path for PowerShell
        const escapedLogFile = logFile.replace(/\\/g, '\\\\');
        
        const psScript = `
$env:CLAUDE_MEM_WORKER_PORT='${port}'
$logPath = '${escapedLogFile}'
$process = Start-Process -FilePath '${bunPath}' -ArgumentList '${script}' -WorkingDirectory '${MARKETPLACE_ROOT}' -WindowStyle Hidden -PassThru -RedirectStandardOutput $logPath -RedirectStandardError $logPath
Write-Output $process.Id
`;

        // Encode to Base64 (UTF-16LE required by PowerShell)
        const psScriptBase64 = Buffer.from(psScript, 'utf16le').toString('base64');

        const result = spawnSync('powershell', ['-EncodedCommand', psScriptBase64], {
          stdio: 'pipe',
          timeout: 10000,
          windowsHide: true
        });
